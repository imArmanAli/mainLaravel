                    ...
                    <div class="box box-danger">
                        <div class="box-header">
                            @if ($superAdmin || in_array(config('permissions.redirect_domain.add'), $permissions))
                                <a href="{{ route('addredirect.show') }}" class="pull-right btn btn-success"><i
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center menu-action">
                                                    @if ($superAdmin || in_array(config('permissions.redirect_domain.edit'), $permissions))
                                                        @if ($nod->rd_status == 0)
                                                            <a href="{{ route('redirect.pause', ['id' => "$nod->rd_id", 'status' => '1']) }}"
                                                        ...
                                                        @elseif($nod->rd_status == 1)
                                                            <a href="{{ route('redirect.play', ['id' => "$nod->rd_id", 'status' => '0']) }}"
                                                                data-original-title="Pause" data-toggle="tooltip"
                                                                data-placement="top" class="btn menu-icon vd_bg-green"> <i
                                                                    class="fa fa-pause"></i> </a>
                                                        @endif
                                                        <a href="{{ route('redirect.edit', "$nod->rd_id") }}"
